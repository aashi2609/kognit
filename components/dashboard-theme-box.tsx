...
export function DashboardThemeBox() {
  ...
  return (
    <div ref={boxRef} className="relative z-50">
      {/* Trigger Button (Classy Glass Pill) */}
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="group flex items-center gap-2.5 rounded-full border border-white/15 bg-slate-950/80 px-4 py-1.5 font-mono text-xs font-bold uppercase tracking-[0.2em] text-slate-200 backdrop-blur-md shadow-sm transition-all duration-300 hover:border-white/30 hover:bg-white/10 hover:text-white"
      >
        <span
          className="h-2.5 w-2.5 rounded-full border border-white/40 shadow-[0_0_6px_rgba(255,255,255,0.3)] transition-transform group-hover:scale-110"
          style={{ backgroundColor: previewColor }}
        />
        <span>Theme</span>
        <span className="text-[9px] text-muted-foreground/60 group-hover:text-white transition-colors">
          {open ? "▲" : "▼"}
        </span>
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-2.5 w-72 rounded-2xl border border-white/15 bg-slate-950/95 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.8)] backdrop-blur-2xl"
          >
